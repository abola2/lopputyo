...
const std::string QUERY_BOOKING_BY_CUSTOMER_NAME = "SELECT Customer.NAME, Bookings.DAY_COUNT, Rooms.BED_AMOUNT from Bookings LEFT JOIN Customer ON Customer.ID=Bookings.CUSTOMER_ID LEFT JOIN Rooms ON Rooms.BookingID=Bookings.BOOKING_ID WHERE UPPER(Customer.NAME)=UPPER(?);";
